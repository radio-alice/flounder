        ...
        if ALLOWED_SCHEMES.contains(&p.scheme()) {
          if p.scheme() == "gemini" {
            ...
            let join = |a: &Url, b: Url| -> Result<String, Box<dyn std::error::Error>> {
              Ok(
                a.join(b.host_str().ok_or("err")?)?
                  .join(b.path())?
                  .as_str()
                  .to_string(),
              )
            };
            let proxied = join(&proxy_url, p).unwrap_or_else(|_| "".to_string()); // Dont fail
...
mod tests {
  ...
  #[test]
  fn test_basic() {
    assert_eq!(gemtext_to_html("hello world"), "hello world<br>\n")
  }
